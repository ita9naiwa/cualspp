...
__global__ void add_reg_kernel(float* YtY, float reg, int d) {
...
__global__ void als_kernel(int *indices, int *indptr, float *data,
...
void load_factors_to_cuda_memory
(float *X, float *Y, int n_users, int n_items, int d, float **dev_X, float **dev_Y) {
    float *_X;
    float *_Y;
...
void _cuALS(int *dev_indices, int *dev_indptr, float *dev_data, int nnz,
    ...
    for(int block_beg = 0; block_beg < d; block_beg += block_size) {
        ...
        als_kernel<<<numSMs * 256, block_size, sizeof(float) * block_size * 4>>>(
